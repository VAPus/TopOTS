 <?php
 if(is_logged() and isset($_GET['id'])){
	...
	$check_owner_q=$mysqli->query('SELECT `owner` FROM `list_ots` WHERE `id`="'.$ID.'"');
	...
	$desc=$server['desc'];
				...
				if(isset($_POST['submit'])){
					...
					$desc=str_replace("</br>","\n",$desc);
					echo '<form autocomplete="off" method="POST" action="?page=edit_serv&id='.$ID.'" enctype="multipart/form-data">
						<div class="form_row">
							...
							<div class="field">
								<div class="fileupload fileupload-new" data-provides="fileupload">
								  ...
								  <div>
									...
									if($remove_img==1){
										echo '<span class="btn btn-file"><a href="?page=edit_serv&id='.$ID.'&action=delete_img">'.$_LANG['edit']['delete'].'</a></span>';
									}
								  echo '
								  </div>
								</div>
